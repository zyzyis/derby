...
create trigger tr1 after update on t1 for each row mode db2sql update t1 set i=i+1;
update t1 set i=i+1;
drop trigger tr1;
create trigger tr1 after update on t1 referencing old as oldt for each row mode db2sql update t1 set i=i+1 where oldt.i=2;
-- ok
update t1 set i=i+1;
select * from t1;
drop trigger tr1;
